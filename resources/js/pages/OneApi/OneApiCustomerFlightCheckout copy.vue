...
const getRelativeDate = ({ years = 0, months = 0, days = 0 }) => {
    const date = new Date();
...
const getDummyTravelerData = (type, index) => {
    const normalizedType = type === "CHD" ? "CNN" : type;
    const base = {
        nationality: "PK",
        issueCountry: "PK",
        documentType: "passport",
        expiryDate: getRelativeDate({ years: 5 }),
        cnic: "",
    };

    if (normalizedType === "INF") {
        return {
            ...base,
            title: "Miss",
            firstName: `Infant${index + 1}`,
            lastName: "Demo",
            documentNo: `INF${String(index + 1).padStart(5, "0")}`,
            dob: getRelativeDate({ years: -1, months: -2 }),
            gender: "F",
        };
    }

    if (normalizedType === "CNN") {
        return {
            ...base,
            title: "Mstr",
            firstName: `Child${index + 1}`,
            lastName: "Demo",
            documentNo: `CHD${String(index + 1).padStart(5, "0")}`,
            dob: getRelativeDate({ years: -8 }),
            gender: "M",
        };
    }
    ...
    return {
        ...
        lastName: "Demo",
        documentNo: `ADT${String(index + 1).padStart(5, "0")}`,
        dob: getRelativeDate({ years: -30 }),
        gender: "M",
        cnic: "35202-1234567-1",
    };
};

const autofillDummyTravellers = () => {
    if (!Array.isArray(travellers.value) || !travellers.value.length) {
        toast.error("No travelers found to autofill.");
        return;
    }

    travellers.value.forEach((traveller, index) => {
        Object.assign(traveller, getDummyTravelerData(traveller.type, index));
        if (errors.travellers?.[index]) {
            Object.keys(errors.travellers[index]).forEach((key) => {
                errors.travellers[index][key] = "";
            });
        }
    });

    globalError.value = "";
    toast.success("Dummy traveler data applied.");
};
                        ...
                        <div class="bg-white shadow-sm border border-gray-200 overflow-hidden">
                            <div class="bg-gray-50 border-b border-gray-200 p-3">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <button type="button" @click="autofillDummyTravellers"
                                        class="bg-primary text-white px-3 py-2 text-xs rounded hover:bg-primary/90 whitespace-nowrap">
                                        Autofill Dummy Travelers
                                    </button>
                                </div>
                            </div>
