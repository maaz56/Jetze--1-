...
function calculateTotalFare(fare) {
  ...
  const total = billable + (agentAmount * passengerCount) + margin - (agentDiscount * passengerCount) + (airportMargin * passengerCount) + extraServicesTotal.value + totalSeatsCost.value;
...
<template>
  <div v-if="bookingDetails" class="min-h-screen bg-gray-50 py-6 print:bg-white print:py-0">
     ...
     <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6 print:hidden">
        ...
        <div class="space-y-4">
          <Button
            ...
            Pay & Confirm Booking ({{ formatAmount(finalPayableAmount) }})
          </Button>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div v-for="(seat, key) in selectedSeats" :key="key" 
            ...
            <div class="text-sm text-gray-600 mt-1">
              Passenger: {{ passengers.find((p, idx) => getPassengerTravelerRef(idx) === seat.travelerRefNumber)?.first_name || 'N/A' }}
            </div>
            ...
            <table class="w-full text-sm border-collapse">
              ...
              <tbody>
                <tr v-for="(pax, i) in getPassengers(bookingDetails[0])" :key="i" class="border-b last:border-0">
                  ...
                  <td class="py-3">
                    <span v-if="getSelectedSeat(processedSeatMapData[0]?.FlightSegmentInfo['@attributes']?.RPH, getPassengerTravelerRef(i))" 
                      class="bg-primary/10 text-primary px-2 py-1 rounded text-xs font-medium">
                      {{ getSelectedSeat(processedSeatMapData[0]?.FlightSegmentInfo['@attributes']?.RPH, getPassengerTravelerRef(i)) }}
                    </span>
  ...
  <div class="bg-white rounded-lg max-w-5xl w-full max-h-screen overflow-y-auto border-2 border-gray-300">
    ...
    <div class="p-6 space-y-10">
      ...
      <div v-for="(segment, segIdx) in ancillarySegments" :key="segIdx" class="border-2 border-blue-200 rounded-lg p-6 bg-blue-50/30">
                ...
                <label v-for="item in getItemsArray(group.AncillaryItems.AncillaryItem)"
                  ...
                  <div class="flex items-center gap-3">
                    <input
                      ...
                      :name="`ancillary-${segIdx}-${gIdx}-${pIdx}`"
    ...
    <!-- Total & Actions -->
    <div class="sticky bottom-0 bg-white border-t border-gray-300 p-5 flex flex-col sm:flex-row justify-between items-center gap-4">
    ...
    <div v-if="isSeatMapOpen"
      ...
      <div class="bg-white rounded-lg max-w-6xl w-full max-h-screen overflow-y-auto border-2 border-gray-300">
            ...
            <div v-for="(passenger, pIdx) in passengers" :key="pIdx" class="mb-8">
              <div class="bg-white p-4 rounded border mb-4">
                <p class="font-medium text-gray-900 mb-3">
                  {{ passenger.title }} {{ passenger.first_name }} {{ passenger.last_name }} ({{ passenger.type }})
                  <span v-if="getSelectedSeat(segment?.FlightSegmentInfo['@attributes']?.RPH, getPassengerTravelerRef(pIdx))" class="text-primary font-semibold ml-2">
                    - Selected: {{ getSelectedSeat(segment?.FlightSegmentInfo['@attributes']?.RPH, getPassengerTravelerRef(pIdx)) }}
                  </span>
                </p>
                ...
                <div class="seat-map-container overflow-x-auto">
                  ...
                  <div class="inline-block min-w-full">
                    ...
                    <!-- Rows -->
                    <div v-for="row in segment?.SeatMapDetails?.CabinClass?.RowInfo" :key="row['@attributes']?.RowNumber" 
                ...
                <!-- Seat Price Info -->
                <div v-if="getSelectedSeatPrice(segment?.FlightSegmentInfo['@attributes']?.RPH, getPassengerTravelerRef(pIdx))" class="mt-3 p-3 bg-primary/10 rounded border border-primary/20">
                  <p class="text-sm font-semibold">
                    Selected Seat: {{ getSelectedSeat(segment?.FlightSegmentInfo['@attributes']?.RPH, getPassengerTravelerRef(pIdx)) }} - 
                    {{ formatAmount(getSelectedSeatPrice(segment?.FlightSegmentInfo['@attributes']?.RPH, getPassengerTravelerRef(pIdx))) }}
                  </p>
                  <p v-if="getSelectedSeatFeatures(segment?.FlightSegmentInfo['@attributes']?.RPH, getPassengerTravelerRef(pIdx))" class="text-xs text-gray-600 mt-1">
                    Features: {{ getSelectedSeatFeatures(segment?.FlightSegmentInfo['@attributes']?.RPH, getPassengerTravelerRef(pIdx)) }}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div class="sticky bottom-0 bg-white border-t border-gray-300 p-5 flex justify-between items-center">
          ...
          <div class="flex gap-3">
            <button @click="closeSeatMap"
              class="px-6 py-3 border-2 border-gray-400 hover:bg-gray-50 font-medium">
              Cancel
            </button>
