...
const visa = computed(() => store.getters["visa/visa"](route.query.visa_id));
...
function deleteVisaHeaderImage(name) {
    store.dispatch("visa/" + DELETE_VISA_HEADER_IMAGES, {
        file_name: name,
    });
}

function updateVisa() {
    store.dispatch("visa/" + UPDATE_VISA, {
...
watch(
    ...
    (newVal) => {
        if (newVal) {
            title.value = visa.value?.title;
            selectedCountry.value = visa.value?.country_name;
            startingPrice.value = visa.value?.starting_price;
            description.value = visa.value?.description;
            selectedCurrency.value = visa.value?.currency;
        }
    },
    { immediate: true }
);
...
<template>
    <div>
        <div class="my-8">
            ...
            <span class="text-3xl font-medium">Update Visa</span>
        </div>
        <form @submit.prevent="updateVisa" class="bg-white rounded-lg p-4">
            <div class="mb-3">
                <Label for="header-image">Header Image</Label>
                <div v-if="visa?.header_image" class="flex flex-wrap items-center gap-2">
                    <Card class="relative w-24 h-24 overflow-hidden group">
                        <img class="w-full h-full object-cover" :src="visa?.header_image" alt="" />
                        <div
                            class="absolute top-0 left-0 bottom-0 right-0 bg-black bg-opacity-50 hidden group-hover:block cursor-pointer">
                            <div class="flex items-center justify-center w-full h-full">
                                <button type="button" @click="deleteVisaHeaderImage(file.name)" class="text-white">
                ...
                <FileUploader max-files="1" @file-uploaded="
                    (file) => {
                        headerImage = file;
                    }
            ...
            <div class="mb-3">
                ...
                <Input type="text" v-model="title" placeholder="Enter your website" id="logo" />
            </div>
            <div class="flex gap-4">
                <div class="mb-3">
                    ...
                    <Popover v-model:open="isOpenCountryDropdown">
                        <PopoverTrigger as-child>
                            <Button variant="outline" role="combobox" class="w-full justify-between py-5">
                                {{
                                    selectedCountry !== ""
                                        ? countries.find(
                                            (country) =>
                        ...
                        <PopoverContent class="w-full p-0">
                            <Command>
                                <CommandInput class="h-9" @input="fetchCountries" placeholder="Search country..." />
                ...
                <div class="mb-3">
                    ...
                    <Input type="text" v-model="startingPrice" placeholder="Enter starting price" id="starting-price" />
                </div>
            </div>
