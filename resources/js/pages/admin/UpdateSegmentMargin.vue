...
const isLoading = computed(() => store.getters["segmentMargin/isLoading"] );
...
const airlineButtonText = computed(() => {
    if (!form.value.airline_ids?.length) return "All Airlines";
...
async function fetchData() {
    try {
        await Promise.all([
            ...
            store.dispatch("segmentMargin/fetchSegmentMargin", route.params.id),
        ]);
        syncAirlineCache(airlines.value);

        const margin = store.getters["segmentMargin/segmentMargin"];
        if (margin) {
            Object.assign(form.value, margin);
            if (Array.isArray(margin.airline_ids)) {
                form.value.airline_ids = margin.airline_ids;
            } else if (margin.airline_id) {
                form.value.airline_ids = [margin.airline_id];
            } else {
                form.value.airline_ids = [];
            }
        }
...
async function updateMargin() {
    ...
    try {
        form.value.airline_id = form.value.airline_ids?.[0] ?? null;
        await store.dispatch("segmentMargin/updateSegmentMargin", {
            id: route.params.id,
            ...form.value
        });
...
<template>
    <section class="min-h-screen p-6 bg-gray-50/50 dark:bg-gray-900/50 pb-20">
        <div class="max-w-[1400px] mx-auto" v-if="!isLoading">
            <div class="flex items-center gap-4 mb-8">
                <Button variant="outline" size="icon" @click="router.back()">
                    <ArrowLeft class="w-4 h-4" />
                </Button>
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Update Segment Margin</h1>
                    <p class="text-gray-500">Edit segment margin rule #{{ route.params.id }}</p>
                </div>
            </div>

            <form @submit.prevent="updateMargin" class="space-y-6">
                        ...
                        <div class="space-y-2 lg:col-span-1">
                            ...
                            <div v-if="selectedAirlines.length" class="flex flex-wrap gap-1 mt-2">
                                <div
                                    ...
                                    class="flex items-center gap-1 bg-emerald-50 text-emerald-700 text-[10px] px-2 py-1 rounded-full border border-emerald-200 font-medium"
                                >
                ...
                <div class="flex justify-center gap-4 mt-12">
                    ...
                    <Button type="submit" class="w-32 h-10 bg-slate-900" :disabled="isSaving">
