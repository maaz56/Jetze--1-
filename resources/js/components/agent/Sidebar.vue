...
const menuItems = [
{ name: "Flights", icon: PlaneTakeoffIcon, route: { name: "DashboardFlights" } },
  { name: "Dashboard", icon: LayoutDashboard, route: { name: "AgentDashboard" } },
  // { name: "Flights", icon: PlaneTakeoffIcon, route: { name: "Flights" } },
  
  { name: "Bookings", icon: ShoppingCartIcon, route: { name: "AgentBookings" } },
  { name: "AgentOfflineBookings", icon: ShoppingCartIcon, route: { name: "AgentOfflineBookings" } },
  { name: "Travellers", icon: UserPlus, route: { name: "Travellers" } },
  { name: "Deposits", icon: CoinsIcon, route: { name: "Deposits" } },
  { name: "Statements", icon: Notebook, route: { name: "AgentLedger" } },
  // { name: "Profit Loss Report", icon: Notebook, route: { name: "AgentProfitLossReport" } },
  { name: "Other Charges", icon: Truck, route: { name: "AgentOtherCharges" } },
];
...
<template>
  ...
  >
    ...
    <div class="flex items-center justify-between h-16 px-4 border-b border-gray-100 bg-white/30">
      <router-link :to="{ name: 'Home' }" class="flex items-center space-x-3">
        ...
        <div v-if="isExpanded" class="flex flex-col">
          <span class="font-semibold text-gray-800 text-sm">{{ user?.name || 'Agent Dashboard' }}</span>
    ...
    <nav class="flex-1 overflow-y-auto py-6 px-3">
      ...
      <!-- Divider -->
      <div v-if="isExpanded" class="my-6 mx-3 h-px bg-gray-300/50"></div>
      
      <!-- Secondary Menu -->
      <div v-if="isExpanded" class="mb-4 px-3">
        <h3 class="text-xs font-medium uppercase text-gray-700 tracking-wider">Account</h3>
      </div>
    </nav>
