...
const margin = ref();
const pnrDetails = ref(null);
const statusResponse = ref(null);
const pnrBookingDetails = computed(() => store.getters["flight/pnrData"]);
...
watch(pnrBookingDetails, () => {
    pnrDetails.value = pnrBookingDetails.value?.bookingDetails;
    statusResponse.value = pnrBookingDetails.value?.itineraryStatus;

    pnr = pnrDetails.value?.Trips?.[0]?.Journey?.[0]?.Segments?.[0]?.Flight?.CRSPNR;
});
...
<section
  v-if="
    statusResponse?.CurrentStatus?.toLowerCase() === 'failed' &&
    statusResponse?.PaymentStatus?.toLowerCase() === 'success'
  "
  class="bg-red-50 border border-red-300 text-red-800 px-6 py-2 "
>
 ...
 <p class="text-sm leading-relaxed">
  We’re sorry, but your booking could not be completed successfully.
  Kindly reach out to our support team so they can assist you further and help resolve the issue.
</p>
                ...
                <div id="print-section">
                    ...
                    print:bg-white print:border-b-2 print:border-primary/80">
                            ...
                            <div class="col-span-1">
                                <img class="h-24 w-24 justify-start ms-7"
                                    :src="agentData?.agent_data?.logo || '/logo.png'" alt="Logo" />
                            </div>
  ...
  {{ pnr && pnr !== '' ? pnr : bookingDetails?.[0]?.itinerary_ref }}
</p>
                        ...
                        <table class="w-full text-xs border-collapse border-2 border-gray-300 print:border-gray-400">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="(traveller, index) in booking?.pessangers" :key="index"
                                    ...
                                    <td class="py-1.5 px-2">
                                        {{ pnrDetails?.booking_details?.[index]?.ticket_no || "-" }}
                                    </td>
                                </tr>
                        ...
                        <div>
                            <div class="overflow-x-auto">
                                ...
                                <table
                                    ...
                                    <thead>
                                        <tr class="bg-gray-50">
                                            <th class="py-1.5 px-2 text-left font-medium text-gray-600">Route</th>
                                    ...
                                    <tbody v-else class="divide-y divide-gray-100 text-sm text-gray-800">
                                        <template
                                            ...
                                            <tr v-for="(fare, fareIndex) in flight.fares.filter(f => {
                                            ...
                                            })" :key="fareIndex" class="hover:bg-gray-50 transition">
                                                <td class="py-2 px-3 font-semibold text-gray-700">
                                                    ✈️ {{ flight?.from?.city?.name || flight?.from?.iata }}
                                                ...
                                                <td class="py-2 px-3 text-gray-700 font-semibold">
                                                    {{ formatAmount(calculateTaxes(fare)) }}
                                                </td>

                                                <td class="py-2 px-3 text-gray-900 font-bold">
