...
<template>
    <header class="sticky top-0 z-50 w-full shadow-lg">
        <div class="relative bg-primary overflow-hidden">
            ...
            <!-- Animated airplane -->
            <svg class="absolute top-2 left-0 w-12 h-12 airplane" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.5">
            ...
            <div class="container mx-auto px-2 sm:px-4 md:px-6 lg:px-8 relative z-10">
                <div class="flex h-14 sm:h-16 items-center justify-between">
                    ...
                    <div class="flex items-center max-sm:hidden">
                        <div class="flex-shrink-0">
                            <div class="flex items-center gap-1 sm:gap-2">
                                ...
                                <span class="text-lg sm:text-xl md:text-2xl font-bold text-white tracking-tight">Jetze</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 sm:gap-4">
                       ...
                       <NotificationsAlert :isAdmin="true" class="text-white"></NotificationsAlert>
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger as-child>
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    class="rounded-full h-10 w-10 sm:h-11 sm:w-11 border border-white/30  hover:bg-white/20 text-white"
                                >
                                    <CircleUser class="h-4 sm:h-5 w-4 sm:w-5" />
                                    <span class="sr-only">Toggle user menu</span>
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" class="w-48 sm:w-56 p-2 bg-black/80 text-white border border-white/30 shadow-lg">
                        ...
                        <div class="flex md:hidden">
                            <Sheet>
                                <SheetTrigger as-child>
                                    <Button variant="ghost" size="icon" class="md:hidden h-10 w-10 sm:h-11 sm:w-11 text-white hover:bg-white/20 rounded-full">
                                ...
                                <SheetContent side="right" class="w-64 sm:w-80 bg-black/90 border-l border-white/30">
                                    <div class="flex justify-center my-4 sm:my-6">
                                        <div class="text-center">
                                            <div class="text-xl sm:text-2xl font-mono font-bold bg-black/50 text-green-400 px-3 sm:px-4 py-1 rounded border border-green-400/30">
                                                {{ currentTime }}
                                            </div>
                                            <div class="text-xs sm:text-sm font-medium text-white mt-2 flex items-center justify-center gap-1">
                                    ...
                                    <nav class="flex flex-col gap-2 mt-4 sm:mt-6">
                                        <Button v-if="authStore.hasPermission('view-dashboard')" variant="ghost" class="justify-start text-sm sm:text-base hover:bg-white/20 text-white py-2" @click="$router.push({name: 'Dashboard'})">
                                            <LayoutDashboard class="mr-2 h-4 sm:h-5 w-4 sm:w-5 text-green-400" />
                                            Dashboard
                                        </Button>
                                        <Button v-if="authStore.hasPermission('manage-staff')" variant="ghost" class="justify-start text-sm sm:text-base hover:bg-white/20 text-white py-2" @click="$router.push({name: 'Staff'})">
                                            <Users class="mr-2 h-4 sm:h-5 w-4 sm:w-5 text-green-400" />
                                            Staff
                                        </Button>
                                        <Button v-if="authStore.hasPermission('manage-airlines')" variant="ghost" class="justify-start text-sm sm:text-base hover:bg-white/20 text-white py-2" @click="$router.push({name: 'Airlines'})">
                                            <Package class="mr-2 h-4 sm:h-5 w-4 sm:w-5 text-green-400" />
                                            Airlines
                                        </Button>
                                        <Button v-if="authStore.hasPermission('view-bookings')" variant="ghost" class="justify-start text-sm sm:text-base hover:bg-white/20 text-white py-2" @click="$router.push({name: 'AdminCustomerBookings'})">
                                            <Truck class="mr-2 h-4 sm:h-5 w-4 sm:w-5 text-green-400" />
                                            Bookings
                                        </Button>
                                        <Button v-if="authStore.hasPermission('manage-settings')" variant="ghost" class="justify-start text-sm sm:text-base hover:bg-white/20 text-white py-2" @click="$router.push({name: 'Setting'})">
