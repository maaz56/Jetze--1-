...
class ZohoController extends Controller
{
    ...
    /**
     * Create an invoice from the booking's locked price snapshot when available.
     */
    public function createInvoice(Request $request)
    {
        $booking = FlightBookings::with(['user.agentData', 'priceSnapshot'])->findOrFail($request->input('booking_id'));
        Log::info($booking);
        ...
        $params = [
            ...
            'amount' => $snapshot?->selling_amount ?? $booking->amount,
            'currency_code' => $snapshot?->selling_currency
                ?? $flightData['leg']['flights'][0]['fares'][0]['currency']['code']
                ?? $flightData['original']['leg']['flights'][0]['fares'][0]['currency']['code'],
            'currency_symbol' => $snapshot?->selling_currency
                ?? $flightData['leg']['flights'][0]['fares'][0]['currency']['symbol']
                ?? $flightData['original']['leg']['flights'][0]['fares'][0]['currency']['symbol'],
        ];
        ...
        try {
            ...
            BookingInvoice::create([
                ...
                'amount' => $snapshot?->selling_amount ?? $booking->amount,
                'currency_code' => $snapshot?->selling_currency ?? $invoice['currency_code'],
