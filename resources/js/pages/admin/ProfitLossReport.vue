...
const grandTotal = computed(() => {
  const total = profitLossReport.value?.data?.reduce((sum, item) => {
    ...
    const airlineMargin = parseFloat(item.total_airline_margin) || 0;
    return sum + profit + airlineMargin;
  }, 0) || 0;
  return Number(total);
});
...
const totals = computed(() => {
  return profitLossReport.value?.data?.reduce(
    (acc, item) => {
      ...
      acc.totalProfit += parseFloat(item.total_profit || 0);
      acc.totalAirlineMargin += parseFloat(item.total_airline_margin || 0);
    ...
    <div class="container mx-auto px-4 py-8 print-landscape border" id="print-section">
      ...
      <section>
        <div>
          ...
          <div>
            <!-- Totals Tiles -->
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6 print:hidden">
              <!-- Total Bookings -->
              <div
                ...
                <div class="z-10 flex items-center">
                  <div>
                    ...
                    <p class="text-3xl font-bold text-primary">{{ grandTotal.toFixed(2) }}</p>
                  </div>
                </div>
              </div>
              <div
              ...
              <div
                ...
                <div class="z-10 flex items-center">
                  <div>
                    <p class="text-sm font-medium text-primary">Booking Amount</p>
                    <p class="text-3xl font-bold text-primary">{{ formatAmount(totals.totalCost) }}</p>
                  </div>
                </div>
              </div>
              ...
              <div
                ...
                <div class="z-10 flex items-center">
                  <div>
                    <p class="text-sm font-medium text-primary">Total Agent Profit</p>
                    <p class="text-3xl font-bold text-primary">{{ formatAmount(totals.totalProfit) }}</p>
                  </div>
                </div>
              </div>

              <!-- Total Airline Margin -->
              <div
                class="relative overflow-hidden bg-primary/20 rounded-xl p-5 flex items-center transform transition-all duration-300 hover:translate-y-[-5px] hover:shadow-lg">
                <div class="z-10 flex items-center">
                  <div>
                    <p class="text-sm font-medium text-primary">Total Airline Margin</p>
                    <p class="text-3xl font-bold text-primary">{{ formatAmount(totals.totalAirlineMargin) }}</p>
                  </div>
                </div>
              </div>
              ...
              <div
                ...
                <div class="z-10 flex items-center">
                  <div>
                    <p class="text-sm font-medium text-primary">Total Cost</p>
            ...
            <table class="min-w-full bg-white border border-gray-200 shadow-md rounded-lg">
              <thead>
                <tr class="text-left bg-gray-100">
                  ...
                  <th class="px-6 py-3 text-sm font-medium text-gray-700">Booking Amount</th>
                  <th class="px-6 py-3 text-sm font-medium text-gray-700">Total Agent Profit</th>
                  <th class="px-6 py-3 text-sm font-medium text-gray-700">Total Airline Margin</th>
                  <th class="px-6 py-3 text-sm font-medium text-gray-700">Total Cost</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, index) in profitLossReport?.data" :key="index" class="border-t">
                  ...
                  <td class="px-6 py-4 text-sm text-gray-800">{{ formatAmount(item?.total_cost) }}</td>
                  <td class="px-6 py-4 text-sm text-green-600">{{ formatAmount(item?.total_profit) }}</td>
                  <td class="px-6 py-4 text-sm text-green-600">{{ formatAmount(item?.total_airline_margin) }}</td>
              ...
              <tfoot class="print:hidden">
                <tr class="bg-gray-50 font-semibold">
                  ...
                  <td class="px-6 py-4 text-gray-800">{{ formatAmount(totals.totalCost) }}</td>
                  <td class="px-6 py-4 text-green-600">{{ formatAmount(totals.totalProfit) }}</td>
                  <td class="px-6 py-4 text-green-600">{{ formatAmount(totals.totalAirlineMargin) }}</td>
                  <td class="px-6 py-4 text-gray-800">{{ formatAmount(totals.totalRevenue) }}</td>
                </tr>
              </tfoot>
            </table>
          </div>
          <!-- Total Profit under main row -->
