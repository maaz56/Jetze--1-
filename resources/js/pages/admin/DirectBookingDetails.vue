...
import { PlaneIcon, ClockIcon, CalendarIcon, UserIcon } from "lucide-vue-next";
import { PrinterIcon, MailIcon, Download, EyeOff } from "lucide-vue-next";
...
import { calculateLayover, getFormattedDates, formatDateTime, getDuration, calculateLayoverTime } from "@/lib/utils";
...
const adminBookingDetails = computed(() => store.getters["flight/adminBookingData"]);
...
function fetchAdminBookingDetails() {
    if (!booking_id) {
        error.value = "No booking ID provided.";
        return;
    }

    store.dispatch("flight/" + FETCH_ADMIN_BOOKING, {
...
const showDialog = ref(false); // To control the dialog visibility
...
watch(
    () => ({
        confirmationId: pnrDetails.value?.flights?.[0]?.confirmationId,
        ticketNumber: pnrDetails.value?.flightTickets?.[0]?.number,
    }),
    (newValues) => {
        airLinePnr.value = newValues.confirmationId || "";
        ticketNo.value = newValues.ticketNumber || "";
    },
        ...
        <div class="min-h-screen print-container">
            ...
            <div class="print:hidden bg-white p-4 flex justify-end space-x-2 max-w-6xl mx-auto" id="topBar">
                <button @click="printBooking"
                    class="px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                ...
                <!-- <button @click="toggleDetailedInfo"
                    class="px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500">
                    ...
                    <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
                        ...
                        <div class="mt-4">
                            ...
                            <input id="approvalInput" type="text" v-model="airLinePnr"
                                class="w-full px-4 py-2 mt-2 text-gray-700 border rounded-md focus:outline-none focus:ring focus:ring-green-300" />
                        </div>
                        <div class="mt-4">
            ...
            <div id="print-section" v-if="adminBookingDetails"
                    ...
                    print:bg-white print:border-b-2 print:border-green-800">
                        ...
                        <div class="flex flex-col text-white px-4 border-l-2 print:text-black print:border-gray-800">
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <p class="font-semibold">{{ adminBookingDetails?.id }}</p>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <p class="">GDS PNR:</p>
                                <p class="font-semibold capitalize">{{ adminBookingDetails?.pnr }}</p>
                            </div>
                            
                            <div class="grid grid-cols-2 gap-4">
                                <p class=" ">Status:</p>
                                <p class="font-semibold capitalize">
                                    {{ adminBookingDetails?.booking_json_data.isTicketed ? "Ticketed" : "Not Ticketed" }}
                                </p>
                            </div>
                    ...
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <div class="space-y-3">
                            <div v-for="(flight, index) in parseFlightData(adminBookingDetails?.booking_json_data)?.flights" :key="index" class="space-y-2">
                            ...
                            <table
                                ...
                                <thead>
                                    <tr class="bg-gray-50">
                                        ...
                                        <th class="py-1.5 px-2 text-left font-medium text-gray-600">Check-In Baggage(KG)
                                        </th>
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    <tr v-for="(traveller, index) in parseFlightData(adminBookingDetails?.booking_json_data)?.travelers" :key="index"
                                        ...
                                        <td class="py-1.5 px-2">
                                            {{
                                                (parseFlightData(adminBookingDetails?.booking_json_data)?.fareOffers &&
                                                    Array.isArray(parseFlightData(adminBookingDetails?.booking_json_data)?.fareOffers) &&
                                                    parseFlightData(adminBookingDetails?.booking_json_data)?.fareOffers[index]?.checkedBaggageAllowance?.totalWeightInKilograms)
                                                || '-'
                                            }}
                                        </td>
                                        <td class="py-1.5 px-2">
                                        ...
                                        <td class="py-1.5 px-2">
                                            {{ (parseFlightData(adminBookingDetails?.booking_json_data)?.flightTickets && Array.isArray(parseFlightData(adminBookingDetails?.booking_json_data)?.flightTickets)
                                                && parseFlightData(adminBookingDetails?.booking_json_data)?.flightTickets[index]?.number) || '-' }}
                                        </td>
                            ...
                            <table
                                ...
                                <tbody class="divide-y divide-gray-100 ">
                                    <tr v-for="(traveller, index) in parseFlightData(adminBookingDetails?.booking_json_data)?.travelers" :key="index"
                                        ...
                                        <td class="py-1.5 px-2 uppercase">
                                            {{ (parseFlightData(adminBookingDetails?.booking_json_data)?.fares?.[index]?.totals?.subtotal || 0) }}
                                        </td>
                                        <td class="py-1.5 px-2 uppercase">
                                            {{ (parseFlightData(adminBookingDetails?.booking_json_data)?.fares?.[index]?.totals?.taxes || 0) }}
                                        </td>
                                        <td class="py-1.5 px-2 uppercase font-bold">
                                            {{ (parseFlightData(adminBookingDetails?.booking_json_data)?.fares?.[index]?.totals?.total || 0) }}
                                        </td>
