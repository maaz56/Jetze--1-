                    ...
                    <table v-if="bookings?.bookings?.length > 0"
                        ...
                        <tbody>
                            <tr v-for="booking in bookings?.bookings" :key="booking.id"
                                ...
                                <td class="px-4 py-3">
                                    {{ booking.itinerary_reference ? booking.itinerary_reference : booking.pnr }}
                                </td>
                                <td class="px-4 py-3">
                                    {{ parseFlightData(booking?.flight_data)?.original?.provider?.name }}
                                    {{ parseFlightData(booking?.flight_data)?.original?.provider?.identifier }}
                                </td>
                                ...
                                <td px-4 py-3>
                                    <div v-if="booking?.booking_source == 1">
                                        <div v-for="(flight, flightIndex) in parseFlightData(booking?.flight_data)?.original?.leg?.flights" :key="flightIndex">
