...
import { FETCH_GROUP_TICKETS } from "@/services/store/actions.type";
import { useStore } from "vuex";

const store = useStore();

const tickets = computed(() => store.getters["groupTicket/tickets"]);

function fetchGroupTickets() {
    store.dispatch("groupTicket/" + FETCH_GROUP_TICKETS);
}
...
<template>
    {{ groupTickets ?? 'adglasdgluib' }}
    <section>
        <div>
            <div class="flex items-center justify-between my-8">
                <span class="text-3xl font-medium">Group Tickets</span>

                <Button @click="$router.push({ name: 'NewGroupTicket' })"
                    >New group ticket</Button
                >
            </div>
            <div
