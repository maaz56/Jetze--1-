<template>
  <div class="max-w-4xl mx-auto bg-white rounded-lg border p-6 space-y-6 shadow-sm">
    <!-- Header -->
    <h2 class="text-2xl font-semibold text-gray-900 mb-4">Edit Hot Deal</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        >
          <span>{{ form.image ? form.image.name : (existingImage ? 'Change Image' : '+ Choose Image') }}</span>
          <input type="file" class="hidden" @change="onImageChange" accept="image/*" />
        </label>
        
        <!-- Current Image Preview -->
        <div v-if="existingImage && !form.image" class="mt-2">
          <img :src="existingImage" alt="Current deal image" class="w-24 h-16 object-cover rounded border" />
...
import { FETCH_AIRPORTS, FETCH_HOT_DEAL, UPDATE_HOT_DEAL } from "@/services/store/actions.type";
...
const fetchHotDeal = async () => {
  ...
  try {
    ...
    if (data) {
      form.from_airport = data.from_airport;
      form.to_airport = data.to_airport;
      form.title = data.title;
      form.tag = data.tag;
      form.original_price = data.original_price;
      form.discounted_price = data.discounted_price;
      ...
      form.focus_keyword = data.seo?.meta_keywords || "";
      form.meta_title = data.seo?.meta_title || "";
      form.meta_description = data.seo?.meta_description || "";
      form.og_title = data.seo?.og_title || "";
      form.og_description = data.seo?.og_description || "";
      form.canonical_url = data.seo?.canonical_url || "";
      form.no_index = data.seo?.no_index || false;
      form.no_follow = data.seo?.no_follow || false;
      existingImage.value = data.image_url;
    }
  } catch (error) {
    toast.error("Failed to load hot deal data");
...
const submit = () => {
  ...
  store.dispatch("hotDeals/" + UPDATE_HOT_DEAL, { id: route.params.id, data: payload })
    .then(() => {
      router.push({ name: 'HotDeals' });
    })
    .catch((error) => {
      applyServerErrors(error);
      toast.error(error?.response?.data?.message || "Failed to update hot deal");
    });
};

// Fetch airports and hot deal data
onMounted(() => {
