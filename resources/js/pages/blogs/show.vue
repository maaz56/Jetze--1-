...
function fetchBlog() {
  store.dispatch('blog/' + FETCH_BLOG, blogId.value)
    .then(response => {
      blog.value = response.data;

      console.log('fetchBlog() called with id:', blogId.value);
...
<template>
 <div class="min-h-screen bg-gray-50">
    <div v-if="blog?.data?.content" class="container mx-auto px-4 py-12 md:py-16">
      <!-- Header Section -->
      <header class="mb-8 md:mb-12">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between text-sm text-gray-600">
          <div class="flex items-center mb-4 md:mb-0">
            ...
            <span>{{ formatDate(blog.data.published_at) }}</span>
          </div>
          <div v-if="blog.data.seo?.meta_keywords" class="flex items-center">
            <span class="font-medium mr-2">Keywords:</span>
            <div class="flex flex-wrap gap-2">
              <span v-for="(keyword, index) in blog.data.seo.meta_keywords.split(',')" :key="index" class="px-3 py-1 bg-gray-200 text-gray-700 rounded-full text-xs">
                {{ keyword.trim() }}
              </span>
            </div>
          </div>
      ...
      <div v-if="blog.data.featured_image" class="mb-8 md:mb-12">
        <img :src="`/storage/${blog.data.featured_image}`" alt="Featured image" class="w-full h-64 md:h-96 object-cover rounded-xl shadow-lg">
      </div>
      ...
      <footer class="mt-12 pt-8 border-t border-gray-200">
        <div class="flex justify-between items-center">
          ...
          <div class="flex space-x-4">
            <button class="text-gray-600 hover:text-blue-500"><ShareIcon class="w-5 h-5" /></button>
            <button class="text-gray-600 hover:text-blue-500"><TwitterIcon class="w-5 h-5" /></button>
            <button class="text-gray-600 hover:text-blue-500"><LinkedInIcon class="w-5 h-5" /></button>
          </div>
        </div>
      </footer>
    </div>
