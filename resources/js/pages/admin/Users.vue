    ...
    <div>
        <div class="bg-white p-8 rounded-lg border">
            <div
                ...
                <div v-if="authUser.role == 'admin' || authUser.role == 'salesman'"
                    class="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                    <Button variant="outline" @click="
                        () => {
                            $router.push({
                                name: authUser.role == 'admin' ? 'NewUser' : 'NewSalesmanUser',
                            });
                        }
                        ...
                        <Table>
                            <TableHeader class="bg-primary">
                                <TableRow>
                                    ...
                                    <TableHead scope="col" class="px-4 py-3">Email Verification</TableHead>
                                ...
                                <TableRow v-for="user in filteredUsers" :key="user.id"
                                    ...
                                    <TableCell class="px-4 py-2">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-xs font-medium px-2 py-0.5">
                                                {{ user?.agent_data?.ceo_contact || "N/A" }}
                                            </span>
                                        </div>
                                    </TableCell>
                                    <TableCell class="px-4 py-2">
                                    ...
                                    <TableCell class="px-4 py-2 space-x-2 whitespace-nowrap">
                                        <button
                                            v-if="authUser.role == 'admin' || authUser.role == 'salesman' || authUser.role == 'reservation'"
                                            @click="
                                                $router.push({
                                                    name: authUser.role == 'admin'
                                                        ? 'UserDetails'
                                                        : authUser.role == 'salesman'
                                                            ? 'SalesmanUserDetails'
                                                            : authUser.role == 'reservation'
                                        ...
                                        <button v-if="authUser.role === 'admin'" @click="
                                            $router.push({
                                        ...
                                        <button v-if="authUser.role === 'admin'" @click="confirmDelete(user)"
